...
class CRNN(nn.Module):
    ...
    def forward(self, X):
        ...
        for t in range(T):
            ...
            print(t, active, len(active))
            
            active = Variable(torch.FloatTensor(active).view(-1,1))

            input = torch.cat([X[t],Variable(torch.ones(1,B))],0)
            
            i_h = torch.mm(Wi,input) # (num_clocks * hidden_state, embedding_size + 1) * (embedding_size + 1, batch_size)
            
            _H_prev = torch.cat([H_prev,Variable(torch.ones((1, B)))],0) # (num_clocks * hidden_state, batch_size) -> (num_clocks * hidden_state + 1, batch_size)
            
            h_h = torch.mm(Wh, _H_prev) # (num_clocks * hidden_state, num_clocks * hidden_state + 1) * (num_clocks * hidden_state + 1, batch_size)
            
            h_new = i_h + h_h # (num_clocks * hidden_state, batch_size)

            H_new = F.tanh(h_new) # (num_clocks * hidden_state, batch_size)

            # active: (num_clocks * hidden_state, 1)
            H = active.expand_as(H_new)*H_new+(1-active).expand_as(H_prev)*H_prev # active 0 -> H_prev; 1 -> H_new
            # H: (num_clocks * hidden_state, batch_size)
            
            _H = torch.cat([H, Variable(torch.ones((1, B)))],0) # (num_clocks * hidden_state + 1, batch_size)

            y = torch.mm(Wo, _H) # (output_size, num_clocks * hidden_state + 1) * (num_clocks * hidden_state + 1, batch_size)
            
            Y = F.tanh(y) # (output_size, batch_size)
